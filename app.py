...
BASE_DIR=Path(__file__).resolve().parent;ENGINE_FILE=BASE_DIR/"tu_vi_engine.json";BOOKS_FILE=BASE_DIR/"books_cache.json";PROMPT_DIR=BASE_DIR/"system_prompts"
...
st.title("☯️ TỬ VI ĐẨU SỐ");st.caption("Quét hình ảnh bằng Python → nhận dạng vị trí → chuẩn hóa Can-Chi/sao/hạn → AI chỉ nhận JSON sạch.")
st.header("① Quét hình ảnh");uploaded=st.file_uploader("Chọn ảnh lá số",type=["jpg","jpeg","png","webp"])
with st.expander("⚙️ Tùy chỉnh vùng quét"):
    top=st.slider("Bỏ lề trên (%)",0,25,0);bottom=st.slider("Bỏ lề dưới (%)",0,25,3);side=st.slider("Bỏ lề trái/phải (%)",0,15,0);overlap=st.slider("Overlap (px)",5,60,28)
if uploaded:
    ...
    if st.button("🔎 QUÉT HÌNH ẢNH",type="primary",use_container_width=True):
        try:
            with st.spinner("Python đang đọc 12 cung bằng nhiều chế độ OCR..."):
                cropped=crop_12_cung(image,top,bottom,side,overlap);raw=extract_chart_text(image,cropped);normalized=normalize_processed_data(raw);chart=build_chart_json(normalized)
                if not isinstance(chart,dict) or not isinstance(chart.get("12_cung"),dict):raise ValueError("Bộ chuẩn hóa OCR không tạo được bảng 12 cung hợp lệ.")
                st.session_state.normalized=normalized;st.session_state.chart_json=chart;st.session_state.validation=validate_chart(normalized);st.session_state.confirmed=False
            st.success("Đã quét và chuẩn hóa dữ liệu. Ảnh không được gửi cho AI.")
        except Exception as exc:
            st.session_state.chart_json=None;st.session_state.normalized=None;st.error(f"Không thể hoàn tất bước quét/chuẩn hóa OCR: {type(exc).__name__}: {exc}")

if st.session_state.chart_json:
    st.header("② Bảng 12 cung sạch");chart=st.session_state.chart_json;rows=[]
    for name,data in chart.get("12_cung",{}).items():
        main="; ".join(x.get("ten","")+(f" ({x['trang_thai']})" if x.get("trang_thai") else "") for x in data.get("chinh_tinh",[])) or "—";sup="; ".join(data.get("phu_tinh",[])) or "—";periods=[]
...
st.divider();st.header("③ Luận giải");col1,col2=st.columns([1,3])
...
if st.session_state.confirmed and st.button("🔮 BẮT ĐẦU LUẬN GIẢI",type="primary",use_container_width=True):
    if prompt_err or engine_err or books_err:st.error("Thiếu System Prompt / Engine / Books.")
        ...
        with st.spinner("Python tính dữ liệu → AI luận theo System Prompt..."):
            calc=calculate_chart(st.session_state.chart_json);st.session_state.analysis_result=generate_analysis(st.session_state.chart_json,calc,year,note)
if st.session_state.analysis_result:
    st.header("④ Kết quả luận giải");render(st.session_state.analysis_result);st.download_button("⬇️ Tải JSON",st.session_state.analysis_result,"analysis.json","application/json")
