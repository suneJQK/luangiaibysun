...
from ocr_engine import crop_12_cung, extract_chart_text, get_grid_preview
...
def generate_analysis(chart, calc, year, note):
    prompt = f"""Năm luận: {year}\nYêu cầu: {note}\n\nDỮ LIỆU LÁ SỐ ĐÃ CHUẨN HÓA BỞI PYTHON (nguồn duy nhất về vị trí và sao):\n{compact(chart, 50000)}\n\nDỮ LIỆU QUAN HỆ TÍNH BỞI PYTHON:\n{compact(calc, 20000)}\n\nENGINE KIẾN THỨC:\n{compact(engine, 70000)}\n\nTÀI LIỆU:\n{compact(books, 50000)}\n\nQUY TẮC: Chỉ sử dụng JSON sạch. Không nhìn ảnh, không dùng raw OCR, không tự thêm sao. 93 trong dai_van.tuoi_bat_dau là tuổi bắt đầu Đại vận. Th.2 là Lưu nguyệt tháng 2. ĐV.X là cung Đại vận; LN.X là cung Lưu niên; Tuần/Triệt là trạng thái cung; can_chi là Can-Chi đầy đủ của cung. Luận đủ chính tinh, phụ tinh, lưu tinh, tứ hóa và sát tinh đã được liệt kê."""
    ...
    try:
        ...
        for x in d.get("luan_12_cung", []):
            with st.expander(x.get("cung", "Cung")):
                st.markdown(x.get("ket_luan", ""))
                st.caption("Căn cứ: " + "; ".join(x.get("can_cu", [])))
        for title, key in [("Đại vận", "dai_van"), ("Tiểu hạn", "tieu_han")]:
            with st.expander(title):
                for x in d.get(key, []):
                    st.write("• " + x)
        st.markdown("### Kết luận")
        for x in d.get("ket_luan", []):
            st.write("• " + x)
        if d.get("canh_bao"):
...
for k, v in [("chart_json", None), ("normalized", None), ("validation", None), ("analysis_result", "")]:
    st.session_state.setdefault(k, v)
st.session_state.setdefault("confirmed", False)
st.title("☯️ TỬ VI ĐẨU SỐ")
st.caption("Quét hình ảnh bằng Python → nhận dạng vị trí → chuẩn hóa Can-Chi/sao/hạn → AI chỉ nhận JSON sạch.")
...
if uploaded:
    ...
    iw, ih = image.size
    st.caption(f"Kích thước ảnh: **{iw} × {ih}px**")

    with st.expander("⚙️ Tùy chỉnh vùng quét 12 cung", expanded=True):
        st.info("Kéo 4 mép để khoanh đúng phần khung 12 cung. Không cần cắt sát từng ô; hệ thống sẽ chia vùng đã chọn thành lưới 4×4 và chỉ OCR 12 ô ngoài.")
        c1, c2 = st.columns(2)
        with c1:
            x_start = st.slider("X trái (%)", 0, 30, 0, 1, help="Mé trái của toàn bộ khung 12 cung.")
            y_start = st.slider("Y trên (%)", 0, 30, 0, 1, help="Mé trên của toàn bộ khung 12 cung.")
        with c2:
            x_end = st.slider("X phải (%)", 70, 100, 100, 1, help="Mé phải của toàn bộ khung 12 cung.")
            y_end = st.slider("Y dưới (%)", 70, 100, 100, 1, help="Mé dưới của toàn bộ khung 12 cung.")
        auto_grid = st.checkbox("Tự dò đường kẻ trong vùng đã chọn", value=False, help="Tắt nếu đường kẻ bị nhầm với chữ; khi tắt, vùng được chia đúng 4 phần bằng nhau.")
        overlap = st.slider("Overlap giữa các ô (px)", 0, 20, 0, 1, help="Nên để 0. Chỉ tăng khi chữ sát đường kẻ bị mất.")

        preview, xs, ys = get_grid_preview(image, x_start=x_start, y_start=y_start, x_end=x_end, y_end=y_end, auto_grid=auto_grid)
        prev = np.asarray(preview).copy() if 'np' in globals() else None
        if prev is None:
            import numpy as np
            prev = np.asarray(preview).copy()
        for x in xs[1:-1]:
            prev[:, max(0, x - 1):min(prev.shape[1], x + 1)] = [255, 0, 0]
        for y in ys[1:-1]:
            prev[max(0, y - 1):min(prev.shape[0], y + 1), :] = [255, 0, 0]
        st.image(prev, caption="Xem trước vùng OCR — đường đỏ là ranh giới các ô", use_container_width=True)
        st.caption(f"ROI hiện tại: X {x_start}%→{x_end}% · Y {y_start}%→{y_end}% · {'dò đường kẻ' if auto_grid else 'chia đều 4×4'}")

    with st.expander("Xem ảnh gốc"):
        st.image(image, use_container_width=True)

    if st.button("🔎 QUÉT HÌNH ẢNH", type="primary", use_container_width=True):
        try:
            with st.spinner("Python đang đọc 12 cung bằng nhiều chế độ OCR..."):
                cropped = crop_12_cung(image, x_start=x_start, y_start=y_start, x_end=x_end, y_end=y_end, overlap_px=overlap, auto_grid=auto_grid)
...
if st.session_state.chart_json:
    ...
    for name, data in chart.get("12_cung", {}).items():
        ...
        rows.append({"Cung": name + (" (Thân cư)" if data.get("than_cu") else ""), "Can-Chi": data.get("can_chi", "—"), "Hành": (("+" if data.get("am_duong") == "Dương" else "-") if data.get("am_duong") else "") + (data.get("ngu_hanh") or "—"), "Trường sinh": data.get("vong_truong_sinh", "—"), "Tuần/Triệt": flags or "—", "Chính tinh": main, "Phụ tinh": sup, "Lưu tinh": ", ".join(data.get("luu_tinh", [])) or "—", "Tứ hóa": ", ".join(data.get("tu_hoa", [])) or "—", "Hạn": " | ".join(periods) or "—"})
