...
def load_prompt():
    ...
    except Exception as exc: return "Bạn là chuyên gia Tử Vi Đẩu Số.", str(exc)
...
# 14 chính tinh. Dùng cả tên chuẩn hóa và ID thực tế trong chinh_tinh.
...
def _canonical_star_name(value) -> str:
    text = "" if value is None else str(value).strip()
    if not text: return ""
    # Xóa [V], [M], [Đ], [H]... và chú thích trong ngoặc.
    text = re.sub(r"\s*\[[^\]]*\]", "", text)
    text = re.sub(r"\s*\([^)]*\)", "", text)
    text = unicodedata.normalize("NFC", text).strip().casefold()
    return re.sub(r"\s+", " ", text)

def _star_name(value) -> str:
    if isinstance(value, dict): return str(value.get("ten") or value.get("saoTen") or "").strip()
    return str(value).strip()

def _filter_phu_stars(main, phu):
    """Chỉ lọc cho bảng 12 cung; JSON engine vẫn giữ nguyên."""
    main_ids = {x.get("id", x.get("saoID")) for x in main if isinstance(x, dict) and x.get("id", x.get("saoID")) is not None}
    main_names = {_canonical_star_name(_star_name(x)) for x in main if _canonical_star_name(_star_name(x))}
    result, seen = [], set()
    for star in phu:
        name = _star_name(star)
        canonical = _canonical_star_name(name)
        sid = star.get("id", star.get("saoID")) if isinstance(star, dict) else None
        if sid in main_ids or canonical in main_names or canonical in MAIN_STAR_NAMES:
            continue
        key = sid if sid is not None else canonical
        if not key or key in seen: continue
        seen.add(key); result.append(star)
    return result

for key, default in [("chart_json", None), ("chat_history", [])]: st.session_state.setdefault(key, default)
...
if lap:
    try:
        ...
        if not isinstance(chart, dict) or len(chart.get("12_cung", {})) != 12: raise ValueError("Engine không tạo đủ 12 cung.")
        chart.setdefault("input", {})["gio_hien_thi"] = gio_label; chart["input"]["lich"] = lich
        st.session_state.chart_json = chart; st.session_state.chat_history = []
        st.success("Đã lập lá số và an sao bằng engine local.")
    except Exception as exc: st.error(f"Không thể lập lá số: {type(exc).__name__}: {exc}")

if st.session_state.chart_json:
    chart = st.session_state.chart_json; tb = chart.get("thien_ban", {})
    st.header("② Dữ liệu lập lá số")
    m = st.columns(5); m[0].metric("Nguồn", "LOCAL"); m[1].metric("12 cung", len(chart.get("12_cung", {}))); m[2].metric("Cục", tb.get("ten_cuc") or "—"); m[3].metric("Mệnh", tb.get("menh") or "—"); m[4].metric("Thân", tb.get("than_chu") or "—")
    ...
    with tabs[1]:
        ...
        for name, data in chart.get("12_cung", {}).items():
            ...
            main = data.get("chinh_tinh") or []; phu = data.get("phu_tinh") or []
            filtered_phu = _filter_phu_stars(main, phu)
            def names(items): return "; ".join(_star_name(x) for x in items if _star_name(x)) or "—"
            rows.append({"Cung": name + (" (Thân cư)" if data.get("than_cu") else ""), "Can-Chi": data.get("can_chi") or "—", "Ngũ hành": data.get("ngu_hanh") or "—", "Tràng sinh": data.get("vong_trang_sinh") or "—", "Tuần/Triệt": ", ".join(flags) or "—", "Chính tinh": names(main), "Phụ tinh": names(filtered_phu)})
        st.dataframe(rows, use_container_width=True, hide_index=True)
    with tabs[2]: st.json({name: data.get("dai_van", {}) | {"tieu_van": data.get("tieu_van", {}), "luu_nien": data.get("luu_nien", {}), "luu_nguyet": data.get("luu_nguyet", {})} for name, data in chart.get("12_cung", {}).items() if isinstance(data, dict)})
    with tabs[3]: st.json(chart)
        ...
        with st.chat_message("assistant"):
            try:
                with st.spinner("AI đang luận giải…"):
                    calc = calculate_chart(chart); answer = ask_ai(question, chart, calc, date.today().year)
                st.markdown(answer); st.session_state.chat_history.append({"role":"assistant","content":answer})
            except Exception as exc:
                answer = f"Không thể gọi AI: {type(exc).__name__}: {exc}"; st.error(answer); st.session_state.chat_history.append({"role":"assistant","content":answer})
else:
