...
def build_chart_json(processed):
    out={"schema_version":"3.2","source":"python_ocr","image_sent_to_llm":False,"12_cung":{}}
    ...
    for palace_key,raw_data in cungs.items():
        ...
        for star in data.get("stars",[]) or []:
            ...
            ns=star["name"]; typ=star.get("type"); obj={"ten":ns}
            ...
            elif typ=="phu_tinh":(entry["luu_tinh"] if star.get("luu") else entry["phu_tinh"]).append(ns)
            elif typ=="hoa_tinh":entry["tu_hoa"].append(ns)
            elif typ=="sat_tinh":entry["sat_tinh"].append(ns)
