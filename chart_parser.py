...
def _as_dict(value):
    """Backward-compatible guard: an older normalizer may return a list."""
    if isinstance(value, dict): return value
    if isinstance(value, list): return {"stars": value}
    return {}

def build_chart_json(processed):
    out={"schema_version":"3.1","source":"python_ocr","image_sent_to_llm":False,"12_cung":{}}
    ...
    for palace_key, raw_data in cungs.items():
        ...
        entry={
            "dia_chi":data.get("dia_chi"),"can":data.get("can"),"ngu_hanh":data.get("ngu_hanh"),"am_duong":data.get("am_duong"),
            "vong_truong_sinh":data.get("vong_truong_sinh"),"tuan":bool(data.get("tuan")),"triet":bool(data.get("triet")),
        ...
        for star in data.get("stars",[]) or []:
            ...
            name_star=star["name"]; typ=star.get("type")
            obj={"ten":name_star}
